...
function buildIntentHintBlock(intentHint?: string): string {
  ...
  switch (mode) {
    ...
    case 'audit':
      return `【当前模式】审计。先读取项目资料并输出问题、证据和风险等级；审计报告必须用 knowledge_save_document 保存到项目知识库（sourceType=audit-report，sourceLabel=story-deep-audit，metadata 写入 auditMode=global-assistant-v2、riskCount/criticalCount 等摘要字段）。审计报告不是项目事实，不得当作 canon 反向注入正文。只有当修法明确且低风险时才产出暂存变更；修正章节/世界观/人物/大纲/创作记忆时使用对应 stage_* 工具。审计应覆盖设定矛盾、人物 OOC、大纲断裂、伏笔未回收和项目约束冲突。`
    default:
      return ''
  }
}
