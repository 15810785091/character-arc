...
function formatChapterContract(value: unknown): string {
  if (!value || typeof value !== 'object') return '暂无；请严格依据章节摘要规划。'
  const contract = value as Record<string, unknown>
  const list = (key: string): string => {
    const items = Array.isArray(contract[key]) ? contract[key] as unknown[] : []
    return items.length ? items.map((item) => `  - ${String(item)}`).join('\n') : '  - 无'
  }
  return [
    `本章目标：${String(contract.goal ?? '') || '未指定'}`,
    `视角人物：${String(contract.pov ?? '') || '未指定'}`,
    `时间地点：${String(contract.timeAndPlace ?? '') || '未指定'}`,
    `核心冲突：${String(contract.conflict ?? '') || '无明确冲突'}`,
    '必须发生：',
    list('mustHappen'),
    '禁止出现：',
    list('forbidden'),
    `结尾钩子：${String(contract.endingHook ?? '') || '未指定'}`
  ].join('\n')
}
...
const handler: TaskHandler = {
  ...
  buildPrompt(input: PromptBuildInput) {
    ...
    const skillsSection = effectiveSkillsBlock ? `\n\n本步骤启用 skills：\n${effectiveSkillsBlock}` : ''
    const userPromptSection = String(context.userPrompt ?? '').trim()
      ? `\n\n本步骤补充要求：${String(context.userPrompt).trim()}`
      : ''
